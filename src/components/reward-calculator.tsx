...
export function RewardCalculator() {
  ...
  const copyToClipboard = () => {
    if (rewardPerSecond !== null) {
      ...
      toast({
        title: "Copied!",
        description: "Reward per second value copied to clipboard.",
      });
    }
  };
          ...
          {rewardPerSecond !== null && (
            <div className="space-y-2 rounded-lg bg-muted p-4">
              <p className="text-sm font-medium">
                Calculated Reward Token / Second (in wei)
              </p>
              <div className="flex items-center gap-4">
                <p className="font-mono text-sm text-foreground break-all flex-grow">
                  {rewardPerSecond.toString()}
                </p>
                <Button variant="ghost" size="icon" onClick={copyToClipboard} disabled={rewardPerSecond === null}>
